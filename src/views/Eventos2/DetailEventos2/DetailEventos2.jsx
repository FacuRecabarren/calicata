...
const DetailEventos2 = () => {
    ...
    useEffect(() => {
        ...
        const loadPaquetesData = async () => {
          let paquetes;
          if (i18n.language === 'es') {
            paquetes = await import('../eventos.json');
          } else if(i18n.language === 'en'){
            paquetes = await import('../eventosEn.json');
          }else{
            paquetes = await import('../eventosPort.json')
          }
    ...
    return (
        <div className='flex flex-col justify-center items-start'>
            <div className='relative w-full'>
                <img loading='lazy' src={paqueteDetail.coverImage} alt="" className='object-cover h-[25rem] w-full -z-10 brightness-75'/>
                <div className='absolute z-10 top-[50%] sm:left-5 flex flex-col sm:justify-start sm:items-start justify-center items-center gap-2 sm:px-0 px-5'>
                    ...
                    <p className='hidden lg:block absolute -bottom-10 text-[#FCCC71] text-xl cursor-default uppercase'><Link to="/recitals" className='z-50 text-white hover:text-[#bcbec7] duration-300 hover:underline'>{t("events")}</Link> / {paqueteDetail.title}</p>
                </div>
            </div>
