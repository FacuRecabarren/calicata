...
const ViajesArgentinaDetail = () => {
    ...
    useEffect(() => {
        const loadPaquetesData = async () => {
          let paquetes;
          if (i18n.language === 'es') {
            paquetes = await import('../viajesEs.json');
      ...
      const handleDescargarArchivo = () => {
        ...
        link.download = 'nombre-del-archivo'; // Nombre que tendrá el archivo descargado
    ...
    return (
        <div className='flex flex-col justify-center items-start'>
            <div className='relative w-full'>
                <img loading='lazy' src={paqueteDetail.coverImage} alt="" className='object-cover h-[25rem] w-full -z-10 brightness-50'/>
                <div className='w-full absolute z-10 top-[50%] sm:left-5 flex flex-col sm:justify-start sm:items-start justify-center items-center gap-2 sm:px-0 px-5'>
                    ...
                    <p className='hidden lg:block absolute -bottom-10 text-[#FCCC71] text-xl cursor-default uppercase'><Link to="/" className='z-50 text-white hover:text-[#bcbec7] duration-300 hover:underline'>{t("home")}</Link> / <Link to="/argentina/travelargentina" className='z-50 text-white hover:text-[#bcbec7] duration-300 hover:underline'>Argentina</Link> / {paqueteDetail.title}</p>
                </div>
            </div>
            <div className='py-10 px-10 w-full sm:flex sm:flex-row flex flex-col justify-between items-start gap-10 sm:gap-16'>
                <section className='w-full sm:w-[40%] flex flex-col justify-center items-start gap-4'>
                    <article className='flex flex-col gap-2 w-full'>
                        ...
                        <p className='text-5xl font-bold text-[#218B7D]'>AR${paqueteDetail.price}</p> */}
                    ...
                    <article className='flex flex-col gap-2 w-full'>
                        ...
                        <div className='flex justify-start items-center gap-2 border border-[#218B7D] rounded-r-xl w-full'>
                                ...
                                <div className='flex justify-between items-center w-full pr-3'>
                                    <p className='opacity-80 font-medium text-sm font-lora italic'>BUS MIX</p>
                                </div>
                            </div>
                        <div className='flex flex-col gap-2'>
                            <div className='flex justify-start items-center gap-2 border border-[#218B7D] rounded-r-xl w-full'>
                            ...
                            <div className='flex justify-start items-center gap-2 border border-[#218B7D] rounded-r-xl w-full'>
                                ...
                                <div className='flex justify-between items-center w-full pr-3'>
                                    <p className='opacity-80 font-medium text-sm font-lora italic'>{paqueteDetail.pension}</p>
                                </div>
                            </div>
                            <div className='flex justify-start items-center gap-2 border border-[#218B7D] rounded-r-xl w-full'>
                                <div className='bg-[#218B7D] text-xl text-white p-2'>
                        ...
                        <p className='font-semibold opacity-80 text-sm'>Salidas desde: {paqueteDetail.from}</p>
                    </article>
                </section>
                <section className='flex flex-col justify-center items-start gap-3 w-full'>
                    ...
                    <div className='flex justify-start items-center gap-3 border border-[#218B7D] rounded-r-xl w-full'>
                        ...
                        <a href={paqueteDetail.itinerary} download>
                            <p className='text-xl font-semibold opacity-80 hover:opacity-50 duration-300'>{t("download")}</p>
                        </a>
...
export default ViajesArgentinaDetail;