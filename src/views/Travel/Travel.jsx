...
const Travel = () => {
    ...
    const ideasByLanguage = {
        'en': [
            ...
            {
                id: 2,
                image: 'https://res.cloudinary.com/dfschbyq2/image/upload/v1711846458/7-maravillas-del-mundo_upscayl_4x_realesrgan-x4plus-anime_w6cw4l.png',
  ...
  return (
    <div>
        <div className='h-[30rem] relative flex flex-col justify-center items-center gap-2'>
            <img src="https://res.cloudinary.com/dfschbyq2/image/upload/v1711922004/Viajar-felicidad_nfqehc.webp" alt="" className='w-full h-full object-cover object-center absolute top-0 -z-10 brightness-75'/>
            <h1 id='title' className='text-5xl lg:text-7xl text-[#ffffff] font-extrabold'>{t("travelUppercase")}</h1>
            ...
            <section className='flex justify-center items-center gap-5 flex-wrap py-10 px-10 bg-[#fe914d43]'>
                {ideas.map(idea => (
                    <div key={idea.id} className='group/item relative flex justify-center items-center lg:hover:scale-105 duration-500'>
                        ...
                        <a href={`/travel/${idea.link}`} className='cursor-alias invisible lg:group/edit group-hover/item:visible absolute flex flex-col justify-center items-center gap-2 bg-opacity-60 bg-[#218B7D] p-10 rounded-full'>
            ...
            {ideas
              ...
              .map((idea) => (
                <div key={idea.id} className='group/item relative flex justify-center items-center lg:hover:scale-105 duration-500'>
                    ...
                    <h3 className='absolute top-0 bg-[#218B7D] w-full text-center py-2 text-lg font-semibold text-white'>{idea.description}</h3>
                    <a href={`/travel/${idea.link}`} className='cursor-alias invisible group/edit lg:group-hover/item:visible absolute flex flex-col justify-center items-center gap-2 bg-opacity-60 bg-[#218B7D] p-10 rounded-full'>
                        <p className='flex justify-center items-center gap-2 font-bold text-white'>{t("consult")}</p>
                    </a>
                </div>
              ))}
